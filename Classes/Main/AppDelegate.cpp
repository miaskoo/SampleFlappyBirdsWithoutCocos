...
bool AppDelegate::applicationDidFinishLaunching() {
    ...
    if(!glview) {
        glview = GLViewImpl::createWithRect("SimpleAngryBirds", Rect(0, 0, 1440, 900));//размер рендера
        director->setOpenGLView(glview);
    }

    director->getOpenGLView()->setDesignResolutionSize(1440, 900, ResolutionPolicy::SHOW_ALL);//размер окна
